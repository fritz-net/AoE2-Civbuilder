...
describe('Complex E2E Workflow Tests', () => {
  ...
  test('should complete full build workflow with mod creation', async () => {
    ...
    // With C++ backend, mod creation should succeed
    // Without C++ backend (test server), endpoint exists but may return 500
    if (useExternalBackend) {
      // When using real C++ backend, expect success
      expect(createResponse.status).toBe(200);
      console.log('Build workflow with C++ backend validated successfully');
    } else {
      // When using test server without C++ backend, accept 200 or 500 but not 404
      expect([200, 500]).toContain(createResponse.status);
      expect(createResponse.status).not.toBe(404);
      
      if (createResponse.status === 500) {
        const errorText = await createResponse.text();
        console.log('Note: Mod creation returned 500 without C++ backend:', errorText.substring(0, 100));
      }
  ...
  test('should complete full draft workflow with mod creation', async () => {
    ...
    // With C++ backend, draft creation should succeed
    // Without C++ backend, endpoint exists but may return 500
    if (useExternalBackend) {

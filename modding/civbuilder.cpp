...
void Civbuilder::createNewUnits() {
    // Crusader Knights
    this->createUU(CRUSADER_KNIGHT, 1723, "Crusader Knight", {600, 0, 0, 1200}, 45, 7604);
    int uuID = (int)(this->df->Civs[0].Units.size() - 2);
    int eID = (int)(this->df->Civs[0].Units.size() - 1);
    for (Civ &civ : this->df->Civs) {
    ...
    // Xolotl Warriors
    createUU(UUCivbuilderID::XOLOTL_WARRIOR, 1570, "Xolotl Warrior", {800, 0, 0, 800}, 60, 7605);
    ...
    // Saboteur
    createUU(SABOTEUR, 706, "Saboteur", {0, 600, 600, 0}, 40, 7606);
    uuID = (int)(this->df->Civs[0].Units.size() - 2);
    eID = (int)(this->df->Civs[0].Units.size() - 1);
    ...
    // Ninja
    createUU(UUCivbuilderID::NINJA, 1145, "Ninja", {0, 500, 0, 600}, 100, 7607);
    uuID = (int)(this->df->Civs[0].Units.size() - 2);
    eID = (int)(this->df->Civs[0].Units.size() - 1);
    int ninjaID = uuID;
    for (Civ &civ : this->df->Civs) {
    ...
    // Flamethrower
    createUU(FLAMETHROWER, 188, "Flamethrower", {0, 1000, 0, 1000}, 75, 7608);
    uuID = (int)(this->df->Civs[0].Units.size() - 2);
    eID = (int)(this->df->Civs[0].Units.size() - 1);
    for (Civ &civ : this->df->Civs) {
    ...
    // Photonman
    createUU(PHOTONMAN, 1577, "Photonman", {1000, 0, 0, 1000}, 120, 7609);
    uuID = (int)(this->df->Civs[0].Units.size() - 2);
    eID = (int)(this->df->Civs[0].Units.size() - 1);
    for (Civ &civ : this->df->Civs) {
    ...
    createUU(APUKISPAY, 1074, "Apukispay", {800, 0, 0, 900}, 70, 7643);
    uuID = (int)(this->df->Civs[0].Units.size() - 2);
    eID = (int)(this->df->Civs[0].Units.size() - 1);
    for (Civ &civ : this->df->Civs) {
    ...
    // Monkey Boy
    createUU(MONKEY_BOY, 860, "Monkey Boy", {2000, 0, 0, 0}, 60, 7612);
    uuID = (int)(this->df->Civs[0].Units.size() - 2);
    eID = (int)(this->df->Civs[0].Units.size() - 1);
    for (Civ &civ : this->df->Civs) {
    ...
    // Amazon Warrior
    createUU(UUCivbuilderID::AMAZON_WARRIOR, 825, "Amazon Warrior", {600, 0, 0, 1000}, 70, 7613);
    uuID = (int)(this->df->Civs[0].Units.size() - 2);
    eID = (int)(this->df->Civs[0].Units.size() - 1);
    for (Civ &civ : this->df->Civs) {
    ...
    // Amazon Archer
    createUU(UUCivbuilderID::AMAZON_ARCHER, 850, "Amazon Archer", {600, 0, 0, 400}, 60, 7614);
    uuID = (int)(this->df->Civs[0].Units.size() - 2);
    eID = (int)(this->df->Civs[0].Units.size() - 1);
    ...
    // Iroquois Warrior
    createUU(IROQUOIS_WARRIOR, 1374, "Iroquois Warrior", {800, 0, 0, 700}, 70, 7615);
    uuID = (int)(this->df->Civs[0].Units.size() - 2);
    eID = (int)(this->df->Civs[0].Units.size() - 1);
    for (Civ &civ : this->df->Civs) {
    ...
    // Varangian Guard
    createUU(VARANGIAN_GUARD, 1681, "Varangian Guard", {900, 0, 0, 900}, 90, 7616);
    uuID = (int)(this->df->Civs[0].Units.size() - 2);
    eID = (int)(this->df->Civs[0].Units.size() - 1);
    for (Civ &civ : this->df->Civs) {
    ...
    // Gendarme
    createUU(GENDARME, 1281, "Gendarme", {1000, 0, 0, 850}, 110, 7617);
    uuID = (int)(this->df->Civs[0].Units.size() - 2);
    eID = (int)(this->df->Civs[0].Units.size() - 1);
    for (Civ &civ : this->df->Civs) {
    ...
    // Cuahchiqueh
    createUU(CUAHCHIQUEH, 1067, "Cuahchiqueh", {600, 0, 0, 900}, 60, 7618);
    uuID = (int)(this->df->Civs[0].Units.size() - 2);
    eID = (int)(this->df->Civs[0].Units.size() - 1);
    for (Civ &civ : this->df->Civs) {
    ...
    // Ritterbruder
    createUU(RITTERBRUDER, 1727, "Ritterbruder", {850, 0, 0, 850}, 60, 7619);
    uuID = (int)(this->df->Civs[0].Units.size() - 2);
    eID = (int)(this->df->Civs[0].Units.size() - 1);
    for (Civ &civ : this->df->Civs) {
    ...
    // Kazak
    createUU(KAZAK, 1269, "Kazak", {0, 1100, 0, 500}, 70, 7620);
    uuID = (int)(this->df->Civs[0].Units.size() - 2);
    eID = (int)(this->df->Civs[0].Units.size() - 1);
    for (Civ &civ : this->df->Civs) {
    ...
    monkPowerUpTasks.push_back(monkPowerUpTask);
    createUU(SZLACHCIC, 1721, "Szlachcic", {750, 0, 0, 650}, 60, 7621);
    uuID = (int)(this->df->Civs[0].Units.size() - 2);
    eID = (int)(this->df->Civs[0].Units.size() - 1);
    for (Civ &civ : this->df->Civs) {
    ...
    // Cuirassier
    createUU(CUIRASSIER, 1186, "Cuirassier", {650, 0, 0, 800}, 60, 7622);
    uuID = (int)(this->df->Civs[0].Units.size() - 2);
    eID = (int)(this->df->Civs[0].Units.size() - 1);
    for (Civ &civ : this->df->Civs) {
    ...
    createUU(RAJPUT, 1184, "Rajput", {750, 0, 0, 750}, 55, 7623);
    uuID = (int)(this->df->Civs[0].Units.size() - 2);
    eID = (int)(this->df->Civs[0].Units.size() - 1);
    for (Civ &civ : this->df->Civs) {
    ...
    // Seljuk Archer
    createUU(SELJUK_ARCHER, 943, "Seljuk Archer", {0, 800, 0, 700}, 65, 7624);
    uuID = (int)(this->df->Civs[0].Units.size() - 2);
    eID = (int)(this->df->Civs[0].Units.size() - 1);
    for (Civ &civ : this->df->Civs) {
    ...
    // Numidian Javelinman
    createUU(NUMIDIAN_JAVELINMAN, 1036, "Numidian Javelinman", {0, 600, 0, 400}, 45, 7625);
    uuID = (int)(this->df->Civs[0].Units.size() - 2);
    eID = (int)(this->df->Civs[0].Units.size() - 1);
    for (Civ &civ : this->df->Civs) {
    ...
    // Sosso Guard
    createUU(SOSSO_GUARD, 1574, "Sosso Guard", {1000, 0, 0, 700}, 65, 7626);
    uuID = (int)(this->df->Civs[0].Units.size() - 2);
    eID = (int)(this->df->Civs[0].Units.size() - 1);
    for (Civ &civ : this->df->Civs) {
    ...
    // Swiss Pikeman
    createUU(SWISS_PIKEMAN, 892, "Swiss Pikeman", {600, 0, 0, 1200}, 45, 7627);
    uuID = (int)(this->df->Civs[0].Units.size() - 2);
    eID = (int)(this->df->Civs[0].Units.size() - 1);
    for (Civ &civ : this->df->Civs) {
    ...
    this->unitClasses["spear"].push_back(eID);
    // Headhunter
    createUU(HEADHUNTER, 1673, "Headhunter", {400, 0, 0, 300}, 50, 7628);
    uuID = (int)(this->df->Civs[0].Units.size() - 2);
    eID = (int)(this->df->Civs[0].Units.size() - 1);
    for (Civ &civ : this->df->Civs) {
    ...
    // Teulu
    createUU(TEULU, 1683, "Teulu", {600, 0, 0, 550}, 45, 7629);
    uuID = (int)(this->df->Civs[0].Units.size() - 2);
    eID = (int)(this->df->Civs[0].Units.size() - 1);
    for (Civ &civ : this->df->Civs) {
    ...
    // Maillotins
    createUU(MAILLOTINS, 1685, "Maillotins", {950, 0, 0, 250}, 35, 7630);
    uuID = (int)(this->df->Civs[0].Units.size() - 2);
    eID = (int)(this->df->Civs[0].Units.size() - 1);
    for (Civ &civ : this->df->Civs) {
    ...
    // Hashashin
    createUU(HASHASHIN, 1035, "Hashashin", {500, 0, 0, 1250}, 60, 7631);
    uuID = (int)(this->df->Civs[0].Units.size() - 2);
    eID = (int)(this->df->Civs[0].Units.size() - 1);
    for (Civ &civ : this->df->Civs) {
    ...
    // Highlander
    createUU(HIGHLANDER, 453, "Highlander", {850, 0, 0, 700}, 65, 7632);
    uuID = (int)(this->df->Civs[0].Units.size() - 2);
    eID = (int)(this->df->Civs[0].Units.size() - 1);
    for (Civ &civ : this->df->Civs) {
    ...
    // Stradiot
    createUU(STRADIOT, 1677, "Stradiot", {800, 0, 0, 850}, 65, 7633);
    uuID = (int)(this->df->Civs[0].Units.size() - 2);
    eID = (int)(this->df->Civs[0].Units.size() - 1);
    for (Civ &civ : this->df->Civs) {
    ...
    // Ahosi
    createUU(AHOSI, 1066, "Ahosi", {450, 0, 0, 350}, 40, 7634);
    uuID = (int)(this->df->Civs[0].Units.size() - 2);
    eID = (int)(this->df->Civs[0].Units.size() - 1);
    for (Civ &civ : this->df->Civs) {
    ...
    landsknechtPowerUpTask.TargetDiplomacy = 4;
    landsknechtPowerUpTasks.push_back(landsknechtPowerUpTask);
    createUU(LANDSKNECHT, 439, "Landsknecht", {850, 0, 0, 650}, 60, 7635);
    uuID = (int)(this->df->Civs[0].Units.size() - 2);
    eID = (int)(this->df->Civs[0].Units.size() - 1);
    for (Civ &civ : this->df->Civs) {
    ...
    // Clibinarii
    createUU(CLIBINARII, 932, "Clibinarii", {950, 0, 0, 850}, 65, 7636);
    uuID = (int)(this->df->Civs[0].Units.size() - 2);
    eID = (int)(this->df->Civs[0].Units.size() - 1);
    for (Civ &civ : this->df->Civs) {
    ...
    // Silahtar
    createUU(SILAHTAR, 1267, "Silahtar", {0, 1100, 0, 650}, 75, 7637);
    uuID = (int)(this->df->Civs[0].Units.size() - 2);
    eID = (int)(this->df->Civs[0].Units.size() - 1);
    for (Civ &civ : this->df->Civs) {
    ...
    // Jaridah
    createUU(JARIDAH, 777, "Jaridah", {900, 0, 0, 450}, 60, 7638);
    uuID = (int)(this->df->Civs[0].Units.size() - 2);
    eID = (int)(this->df->Civs[0].Units.size() - 1);
    for (Civ &civ : this->df->Civs) {
    ...
    // Wolf Warrior
    createUU(WOLF_WARRIOR, 702, "Wolf Warrior", {800, 0, 0, 700}, 65, 7639);
    uuID = (int)(this->df->Civs[0].Units.size() - 2);
    eID = (int)(this->df->Civs[0].Units.size() - 1);
    for (Civ &civ : this->df->Civs) {
    ...
    // Warrior Monk
    createUU(WARRIOR_MONK, 1178, "Warrior Monk", {800, 0, 0, 750}, 80, 7640);
    uuID = (int)(this->df->Civs[0].Units.size() - 2);
    eID = (int)(this->df->Civs[0].Units.size() - 1);
    for (Civ &civ : this->df->Civs) {
    ...
    // Castellan
    createUU(CASTELLAN, 1718, "Castellan", {700, 0, 0, 900}, 75, 7641);
    uuID = (int)(this->df->Civs[0].Units.size() - 2);
    eID = (int)(this->df->Civs[0].Units.size() - 1);
    ...
    // Wind Warrior
    createUU(WIND_WARRIOR, 749, "Wind Warrior", {600, 0, 0, 900}, 65, 7642);
    uuID = (int)(this->df->Civs[0].Units.size() - 2);
    eID = (int)(this->df->Civs[0].Units.size() - 1);
    for (Civ &civ : this->df->Civs) {
